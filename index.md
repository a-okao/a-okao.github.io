...
- 3DCAD (Fusion/Inventor)
    ...
    - 自身のスキルを示したいと思い、3次元CAD利用技術者試験2級を2024年10月に取得しました。
...
- 工作機械
    - フライス盤/旋盤/ボール盤
        ...
        - アルミニウム製の汎用フレーム・薄板に対してのけがき、加工、後処理の流れを習得しています。
    - 3Dプリンター
        - ★★★
        - 使用歴：2年
...
# 製作物
- Fraxinus 1t
    - Fraxinusプロジェクトにて作製しました。
    - CoreXY構造で150mm/s、10000mm/s^2で印刷をすることができます。

- 自作3Dプリンター
    - 出先での印刷や、3Dプリンターのデモンストレーションができたら面白そうだなと思い、作製しました。
    - ハードウェア部分を自身で設計しました。ソフトウェア部分オープンソースの制御ソフトを使用しています。
    - 本体サイズはXX×XX×XXmm、重量はXXkgです。

- 自作キーボード
    - 15キーから成るテンキーを設計しました。
    - 基板はKiCadを用いて図面を作製し、JLCPCBに発注しました。
    - ケースはFusionと3Dプリンターを用いて作製しました。
    - QMKと呼ばれるファームウェアを使用し、自由にキーマッピングができます。

- 在庫管理システム
    - 自身が行っている3Dプリント品販売で、在庫管理を行いたいと思い、作製しました。
    - Raspberry Pi4・LCDディスプレイ・バーコードリーダーから構成されています。
    - バーコードを読み込むことで、在庫数を増減させることができます。
    - 筐体と電装部分を作製しました。ソースコードの部分はAIを使用しました。
